...
def create_sample_shapefile(tmpdir):
    ...
    with fiona.open(str(shapefile_path), 'w', driver='ESRI Shapefile', schema=schema, crs=from_epsg(4326)) as layer:
        layer.write({
...
def test_generate_item(stac_generator):
    ...
    for item in stac_generator.items:
        ...
        assert actual_bbox == expected_bbox, f"Expected bbox: {expected_bbox}, but got: {actual_bbox}"
        ...
        for actual_coord, expected_coord in zip(item.geometry['coordinates'][0], expected_coords):
            assert pytest.approx(actual_coord, rel=1e-9) == expected_coord, f"Expected coord: {expected_coord}, but got: {actual_coord}"
...
def test_generate_collection(stac_generator):
    ...
    assert isinstance(collection, pystac.Collection), "Generated object should be a STAC Collection."
    ...
    assert collection_bbox == expected_bbox, f"Expected bbox: {expected_bbox}, but got: {collection_bbox}"
...
def test_normalize_hrefs(stac_generator):
    ...
    try:
        ...
        pytest.fail(f"normalize_hrefs raised an exception: {str(e)}")
