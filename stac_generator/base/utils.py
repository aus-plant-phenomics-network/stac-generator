...
from stac_generator.csv.utils import calculate_geometry
...
def geometry_from_dict(item: dict[str, Any]) -> Geometry:
    ...
    if "type" != "GeometryCollection" and "coordinates" not in item:
        raise ValueError("Invalid geojson geometry object: no coordinates field for non GeometryCollection")
    ...
    match geometry_type:
        ...
        case "GeometryCollection":
            if "geometries" not in item:
                raise ValueError("Invalid geojson geometry object: no geometries field for GeometryCollection type")
...
def extract_spatial_extent(items: list[pystac.Item]) -> pystac.SpatialExtent:
    ...
    for item in items:
        if (geo := item.geometry) is not None:
            geometries.append(calculate_geometry(geo))
    geo_series = gpd.GeoSeries(data=geometries)
    bbox = geo_series.total_bounds()
    return pystac.SpatialExtent(bbox)


def extract_temporal_extent(items: list[pystac.Item], collection: StacCollectionConfig | None = None) -> pystac.TemporalExtent:
    if collection:
    ...
    for item in items:
        if "start_datetime" in item.properties and "end_datetime" in item.properties:
            ...
            if item.datetime is None:
                raise ValueError("Invalid pystac item. Either datetime or start_datetime and end_datetime values must be provided")
